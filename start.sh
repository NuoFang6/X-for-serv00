...
export ARGO_AUTH=eyJhIjoiZmJhNzQyOGI2OWRiNTdhOTMwODNhMmNhNzUxZWUwMzIiLCJ0IjoiN2NmZjk0ZTMtYWI4OC00YTBjLTk3M2MtZDNiZWUzZjFmODkwIiwicyI6Ik1qUmlOekZoTlRBdE5qTTFNeTAwWmpnMExUaGpaRGd0TVRGall6bGpaalF6TjJaaSJ9
...
mkdir -p ${WORKDIR}
cd ${WORKDIR} && \
[ ! -e ${WORKDIR}/entrypoint.sh ] && wget https://raw.githubusercontent.com/NuoFang6/X-for-serv00/main/entrypoint.sh -O ${WORKDIR}/entrypoint.sh && chmod +x ${WORKDIR}/entrypoint.sh && \
[ ! -e ${WORKDIR}/server.js ] && wget https://raw.githubusercontent.com/NuoFang6/X-for-serv00/main/server.js -O ${WORKDIR}/server.js && \
[ ! -e ${WORKDIR}/package.json ] && wget https://raw.githubusercontent.com/NuoFang6/X-for-serv00/main/package.json -O ${WORKDIR}/package.json
